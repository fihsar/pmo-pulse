...
export async function POST(req: NextRequest) {
  ...
  if (error) return NextResponse.json({ status: 'task_load_failed' }, { status: 500 });
  if (!tasks || tasks.length === 0) return NextResponse.json({ status: 'no_tasks' });
  ...
  try {
    await sendEmail({ to: stakeholderEmails, subject: `Weekly PMO Status — ${rag}`, htmlBody });
